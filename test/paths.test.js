...
test("user-scope paths live under the home directory", () => {
  ...
  assert.equal(userStateFile(), path.join(os.homedir(), ".claude", "ai-toolbox-installed.json"));
...
test("repo-scope paths live under the given repo root", () => {
  ...
  assert.equal(repoStateFile(repoRoot), path.join(repoRoot, ".claude", "ai-toolbox-installed.json"));
